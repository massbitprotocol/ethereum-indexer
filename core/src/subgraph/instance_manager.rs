...
const BUFFERED_BLOCK_STREAM_SIZE: usize = 10;
    ...
    C: Blockchain,
{
    ...
    loop {
        ...
        let stream = new_block_stream(stream_inputs, filter, metrics).await?;
        let mut stream = BufferedBlockStream::spawn_from_stream(stream, BUFFERED_BLOCK_STREAM_SIZE)
        ...
        loop {
            let (block, cursor) = match stream.next().await {
